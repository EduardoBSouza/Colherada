    ...
    try:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS dados_diarios (
                ...
                receitas_anotacoes TEXT DEFAULT '',
...
def login():
    """Rota de login"""
    try:
        ensure_db()  # Garante que o banco está inicializado
...
def obter_dados():
    ...
    try:
        ...
        if not dados_dia:
            ...
            cursor.execute('''
                INSERT INTO dados_diarios (data, estoque, faturamento_bruto, lucro_liquido, receitas_anotacoes)
                VALUES (?, 25, 0, 0, '')
            ''', (data_hoje,))
            conn.commit()
        ...
        resposta = {
            ...
            'receitasAnotacoes': dados_dia.get('receitas_anotacoes', ''),
            'vendas': [
                {
                    'hora': v['hora'],
...
def salvar_dados():
    ...
    try:
        ...
        cursor.execute('''
            INSERT OR REPLACE INTO dados_diarios (data, estoque, faturamento_bruto, lucro_liquido, receitas_anotacoes, atualizado_em)
...
def gerar_relatorio():
    """Gera relatório detalhado por data ou período"""
    if not verificar_autenticacao():
        return jsonify({'error': 'Não autenticado'}), 401
    ...
    try:
        ...
        if not data_fim:
            data_fim = data_inicio  # Se não especificar fim, usa mesma data
        ...
        dados_dias = [dict(row) for row in cursor.fetchall()]
        ...
        total_faturamento = sum(d.get('faturamento_bruto', 0) for d in dados_dias)
        total_lucro = sum(d.get('lucro_liquido', 0) for d in dados_dias)
        ...
        for venda in vendas:
            ...
            if forma not in vendas_por_pagamento:
                vendas_por_pagamento[forma] = {'quantidade': 0, 'total': 0, 'count': 0}
            vendas_por_pagamento[forma]['quantidade'] += venda['quantidade']
            vendas_por_pagamento[forma]['total'] += venda['total']
            vendas_por_pagamento[forma]['count'] += 1
